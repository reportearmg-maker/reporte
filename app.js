(() => {
  ...
  const els = {
    ...
    modalBypass: U.$('modalBypass'),
    selectBypassRmg: U.$('selectBypassRmg'),
    btnConfirmarBypass: U.$('btnConfirmarBypass'),
    btnCancelarBypass: U.$('btnCancelarBypass'),
    btnCerrarModalBypass: U.$('btnCerrarModalBypass'),
  ...
  function inicializarListas() {
    ...
    if (els.selectBypassRmg) U.llenarSelect(els.selectBypassRmg, CONFIG.rmgs, 'Seleccione RMG');
  ...
  function abrirModalUsoBypass() {
    const operador = U.normalizarTexto(els.operador.value);
    if (!operador) {
      U.mostrarMensaje('Para registrar uso de bypass debes seleccionar el operador activo.', 'error');
      els.operador.focus();
      return;
    }

    if (!els.modalBypass || !els.selectBypassRmg) {
      U.mostrarMensaje('No se encontró la ventana para seleccionar la RMG.', 'error');
      return;
    }

    els.selectBypassRmg.value = '';
    els.modalBypass.hidden = false;
    els.modalBypass.setAttribute('aria-hidden', 'false');
    setTimeout(() => els.selectBypassRmg.focus(), 50);
  }

  function cerrarModalUsoBypass() {
    if (!els.modalBypass) return;
    els.modalBypass.hidden = true;
    els.modalBypass.setAttribute('aria-hidden', 'true');
    if (els.selectBypassRmg) els.selectBypassRmg.value = '';
  }
  ...
  async function generarReporteUsoBypass() {
    ...
    if (!operador) {
      ...
      cerrarModalUsoBypass();
      els.operador.focus();
      return;
    }

    const rmgBypass = U.normalizarTexto(els.selectBypassRmg?.value);
    if (!rmgBypass) {
      U.mostrarMensaje('Selecciona la RMG donde se está usando el bypass.', 'error');
      els.selectBypassRmg?.focus();
      return;
    }
    ...
    const reporte = {
      ...
      descripcion: `Registro automático de uso de bypass en ${rmgBypass} generado desde el botón rápido.`,
      accionTomada: `Se registra apertura y cierre del uso de bypass en ${rmgBypass}.`,
      observacionCierre: `Reporte de uso de bypass cerrado automáticamente para ${rmgBypass}.`,
      horaInicio: ahora,
      horaFin: ahora,
    ...
    const boton = els.btnConfirmarBypass || els.btnUsoBypass;
    ...
    try {
      ...
      U.mostrarMensaje(`Reporte de uso de bypass ${guardado.numeroFalla || ''} generado y cerrado correctamente para ${rmgBypass}.`, 'success');
    } catch (error) {
  ...
  async function iniciar() {
    ...
    els.btnUsoBypass.addEventListener('click', abrirModalUsoBypass);
    els.btnConfirmarBypass?.addEventListener('click', generarReporteUsoBypass);
    els.btnCancelarBypass?.addEventListener('click', cerrarModalUsoBypass);
    els.btnCerrarModalBypass?.addEventListener('click', cerrarModalUsoBypass);
    els.modalBypass?.addEventListener('click', event => {
      if (event.target === els.modalBypass) cerrarModalUsoBypass();
    });
    document.addEventListener('keydown', event => {
      if (event.key === 'Escape' && els.modalBypass && !els.modalBypass.hidden) cerrarModalUsoBypass();
    });
