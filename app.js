(() => {
  ...
  function agregarFalla() {
    ...
    grid.append(
      crearCampoSelect('Equipo', 'rmg', CONFIG.rmgs, id, 'Seleccione equipo'),
  ...
  function validarReporte(reporte, id) {
    ...
    if (!reporte.rmg) faltantes.push('equipo');
  ...
  function obtenerRmgsParaBypass() {
    const lista = Array.isArray(CONFIG.rmgBypass) && CONFIG.rmgBypass.length
      ? CONFIG.rmgBypass
      : (CONFIG.rmgs || []).filter(equipo => /^RMG\s+\d+$/i.test(String(equipo).trim()));
    return lista.length ? lista : (CONFIG.rmgs || []);
  }

  function asegurarModalBypass() {
    let modal = U.$('modalBypass');
    if (modal) return modal;

    modal = document.createElement('div');
    modal.id = 'modalBypass';
    modal.className = 'bypass-modal-backdrop';
    modal.hidden = true;
    modal.innerHTML = `
      <div class="bypass-modal-card" role="dialog" aria-modal="true" aria-labelledby="modalBypassTitle">
        <div class="bypass-modal-header">
          <div>
            <h3 id="modalBypassTitle">Uso de bypass</h3>
            <p>Selecciona la RMG donde se utilizará el bypass.</p>
          </div>
        </div>
        <div class="bypass-modal-body">
          <label for="selectRmgBypass">RMG para reporte de bypass</label>
          <select id="selectRmgBypass"></select>
          <small id="mensajeBypassModal">Este reporte quedará cerrado automáticamente con apertura y cierre en la misma hora.</small>
        </div>
        <div class="bypass-modal-actions">
          <button class="btn btn-light" id="btnCancelarBypassModal" type="button">Cancelar</button>
          <button class="btn btn-warning" id="btnConfirmarBypassModal" type="button">Generar reporte</button>
        </div>
      </div>
    `;

    document.body.appendChild(modal);
    U.llenarSelect(U.$('selectRmgBypass'), obtenerRmgsParaBypass(), 'Seleccione RMG');
    return modal;
  }

  function solicitarRmgBypass() {
    return new Promise(resolve => {
      const modal = asegurarModalBypass();
      const select = U.$('selectRmgBypass');
      const mensaje = U.$('mensajeBypassModal');
      const btnCancelar = U.$('btnCancelarBypassModal');
      const btnConfirmar = U.$('btnConfirmarBypassModal');

      select.value = '';
      mensaje.textContent = 'Este reporte quedará cerrado automáticamente con apertura y cierre en la misma hora.';
      mensaje.classList.remove('modal-error');
      ...
      const confirmar = () => {
        const rmg = U.normalizarTexto(select.value);
        if (!rmg) {
          mensaje.textContent = 'Debes seleccionar la RMG antes de generar el reporte de bypass.';
          mensaje.classList.add('modal-error');
          select.focus();
          return;
        }
        cerrar(rmg);
  ...
  async function generarReporteUsoBypass() {
    ...
    const rmgBypass = await solicitarRmgBypass();
    if (!rmgBypass) {
      U.mostrarMensaje('Reporte de bypass cancelado. No se seleccionó RMG.', 'info');
      return;
    }
    ...
    const reporte = {
      ...
      descripcion: `Registro automático de uso de bypass generado para ${rmgBypass}.`,
    ...
    try {
      ...
      U.mostrarMensaje(`Reporte de uso de bypass ${guardado.numeroFalla || ''} generado para ${rmgBypass} y cerrado correctamente.`, 'success');
    } catch (error) {
  ...
  function exportar() {
    ...
    U.exportarReportes(activos, 'reporte_fallas_rmg_3.4.xlsx');
